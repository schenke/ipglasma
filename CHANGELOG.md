...
* Extend the unit test suite with near-complete per-function coverage of the classes above (`Matrix::sqrtm`/`logmPade`/`conjg`/`prodABconj`/`prodAconjB`/`traceOfProdcutOfMatrix`/`expmCoeff`/`square`/`setRe`/`setIm`/`MatrixToString`, `Random`'s `genrand64_*`/`gslRandomInit`/`poisson`/`setGammaIncCDF`/`sampleGammaInc`, `Setup::dFind`/`listFind`, `Glauber::calcRho` and each density profile's `anum*` normalization integral, `Parameters::loadPosteriorParameterSetsFromFile` and a representative sample of its int-to-bool coercing setters, and `FFT::fftnArray`/`fftn`/`fftnVector`), plus new coverage for `Group`'s SU(3) generator normalization (needed to test `Matrix::expmCoeff` against an independent reference), `Cell`'s zero-initializing constructor, `Util`'s namespace-level `stringFind`/`dFind`/`iFind`/`isFile`/`vector_malloc`/`char_malloc`, `PrettyOstream::getMemoryUsage`, `ipg::wallSeconds`, and a sanity check on `Fragmentation::kkp`. Deliberately not covered: `Matrix`'s declared-but-never-defined `operator-`(unary)/`operator/`(binary) (see Fixed), trivial pass-through getter/setter pairs with no branching logic, and anything needing a fully-initialized `Lattice`/multi-nucleus `Glauber::initGlauber`/full JIMWLK or CYM evolution -- that is what `validations/*.py` is for.
* Extend the unit test suite further with `SU3` (`multiply`/`multiplyABdagger`/`commutator`/`trace`/`traceAB`/`traceABdagger`/`traceSquare`/`traceDifferenceSquare`/`traceABC`/`traceABCD`, all cross-checked against the already-tested `Matrix` operators rather than hand-derived reference numbers), `Lattice` (constructor field/neighbor-index allocation, `IsValidWilsonLineDataFormat`, `writeWilsonLines` in both text and binary format, `writeSU3Matrices`, `BufferLattice`), `Group`'s const `getT()` overload, and `Setup::uLLIFind`, closing every remaining gap found while auditing per-function coverage of `Cell`, `FFT`, `Group`, `Instrumentation`, `Lattice`, `Matrix`, `Parameters`, `Random`, `Setup`, `Util` and `SU3`.
* Add a `.github/workflows/clang-format.yml` GitHub Actions workflow that runs `clang-format` on every PR opened from a branch in this repository and pushes a `style: apply clang-format` fixup commit if anything needed reformatting.
...
* Add `FFT::fftnComplexArray` and use it for the Wilson-line construction FFTs in `Init.cpp` (previously `Nc^2-1` separate `fftnComplex` calls in a serial loop), batching and parallelizing them the same way as the `fftnArray`/`fftn` change above; remove `fftnComplex`, now unused. A further ~4% speedup, with bit-identical output; combined with the two changes above, ~33% faster overall than before this round of FFT work.
...
* Replace essentially every remaining raw `cout`/`cerr` print across the codebase (`Evolution`, `FFT`, `Fragmentation`, `GaugeFix`, `Glauber`, `Init`, `JIMWLK`, `Matrix`, `MyEigen`, `Parameters`, `Setup`, `Util`, `main.cpp`'s `readInput`) with `PrettyOstream`, matching the convention already used in `Lattice`/`Init`. Adds a `PrettyOstream messager_` member to each of `Evolution`, `FFT`, `GaugeFix`, `Glauber`, `JIMWLK`, `MyEigen`, `Parameters` and `Setup` (none of these classes are ever copied, moved, or held by value anywhere in the codebase, so their now cannot-be-copied/moved status -- `PrettyOstream` holds a `std::ostringstream`, which is neither -- is not a behavior change); `Util`/`Fragmentation` (free-function namespaces) and every print inside an `#pragma omp parallel`/`#pragma omp for` region instead construct a fresh, stack-local `PrettyOstream` at the call site, since the class has no internal locking and sharing one across threads would be a data race. `main.cpp`'s `display_logo()` banner and `PrettyOstream.cpp`'s own `info`/`debug`/`warning`/`error` implementations are deliberately left as raw `cout`, since a "[Info] X MB" prefix would break the banner's formatting and `PrettyOstream` obviously cannot log through itself.
* Make every `PrettyOstream` log message across the codebase start with a consistent `[ClassName::functionName]` (or `[Namespace::functionName]`/`[main::functionName]`) tag, replacing the handful of ad hoc, inconsistent formats that had accumulated (`[Parameters]` with no function name, `[Init.cpp:readNuclearQs]`/`[Init:getNuclearQs2]` file-based and inconsistently-placed variants, or no tag at all on the vast majority of messages). A handful of genuinely unclear or wrong messages were reworded in the same pass (e.g. `Init::sampleTA`'s dangling, incomplete "Retrieving nuclei from " sentence; a duplicate "Q_s^2(min) S_T" label in `Init::computeCollisionGeometryQuantities` that printed two different variables under the same text; a typo in `Init::readVFromFile`).
* Rename `Init::findUInForwardLightconeChun` to `Init::findUInForwardLightcone`: the "Chun" suffix distinguished it from a slower, already-removed `findUInForwardLightconeBjoern` alternative, so it's no longer needed now that this is the only implementation.
* Reformat the entire `src/` tree with `clang-format` (no functional change), and add a GitHub Actions workflow (`.github/workflows/clang-format.yml`) that reformats and pushes a fixup commit to same-repository PR branches automatically (GitHub's default token cannot push to a fork, so PRs from forks still need `./formatCode.sh` run locally before merging).
...
* Fix `Setup::stringFind`/`stringFind` (Util.cpp) returning `(0)` from a function returning `std::string` on their unreachable-after-`exit(1)` fallback path; the integer literal was implicitly converted via the null-pointer `std::string(const char*)` constructor, which is undefined behavior. Both now return an empty string.
* Change `Nucleus::A`/`Z` (and `Glauber`'s `currentA1_`/`currentA2_`/`currentZ1_`/`currentZ2_`) from `double` to `int`: every assignment site sets a whole-number mass/atomic number literal, and every external caller (`Glauber::nucleusA1/A2/Z1/Z2`) already casts back to `int`, so storing them as `double` only left `Glauber::interNuPInSP`/`interNuTInST`'s `== 1.0` checks as dangerous floating-point comparisons for no reason.
* Fix `PrettyOstream::getMemoryUsage` returning `(0)` from a function returning `std::string` on its (`getrusage`-failure) fallback path -- the same `return (0)` undefined-behavior pattern already fixed elsewhere in `Setup`/`Util`'s `stringFind`, found while writing unit tests for it. Now returns an empty string.
* Fix `Init::readVFromFile`'s two "wrong lattice size"/"wrong grid length" error messages never actually being printed: they built the message text but never called `flush()` before `exit(0)`, so the process just silently exited with no explanation. Also fix `Init::sampleImpactParameter`'s `useNucleus=0` branch flushing its message immediately while the other two branches (linear/uniform sampling) defer to a later, shared flush -- inconsistent timing that would have put a mismatched tag on the shared "b = ..." line for that one branch; all three now defer consistently.
...
* Remove `Matrix::operator==`/`operator!=`: unused anywhere in the codebase and did a direct floating-point equality comparison on `complex<double>` elements.
* Remove `Matrix`'s declared unary `operator-` and binary `operator/(const Matrix&, const Matrix&)`: neither had a definition anywhere in `Matrix.cpp` and neither was ever called, so calling either would have failed to link.
* Remove the old serial validation script and other now-unused files.
* Remove dead, commented-out debug `cout`/`cerr` printouts (and, in a couple of cases, the small dead computations that existed only to feed them) scattered across `FFT.cpp`, `Fragmentation.cpp`, `Glauber.cpp`, `Init.cpp` (including a whole obsolete MPI-broadcast-based alternate `readNuclearQs` implementation, long superseded by the active one above it), `Matrix.cpp` and `Setup.cpp`; remove the `using std::cerr;`/`cout`/`endl` declarations in `main.cpp`/`Matrix.cpp` left unused once nothing in those files used them directly anymore.
