...
// NOTE: this always uses the "3 flavors, local Qs" alphas formula,
// unlike writeEpsilonIntermediatePlot/computeRunningCouplingGfactor, which
// only use it when getRunWithLocalQs()==1 and otherwise fall back to a
// formula based on the averaged Qs getters. Preserved exactly as found;
// this looks like a pre-existing inconsistency between the two writers,
// not something to silently change while extracting this out of run().
void Evolution::writeEpsilonInitialPlot(Lattice *lat, Parameters *param) {
    const int N = param->getSize();
    const double L = param->getL();
    const double a = L / N;  // lattice spacing in fm
    const double g = param->getg();
    const double muZero = param->getMuZero();
