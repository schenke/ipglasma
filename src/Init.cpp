...
#pragma omp parallel
  ...
  if (param->getWriteInitialWilsonLines()>0)
  {
   ...
   strVOne_name << "V-" <<  param->getEventId() + 2*param->getSeed()*param->getMPISize();
   ...
   strVTwo_name << "V-" <<  param->getEventId() + (1+2*param->getSeed())*param->getMPISize();
   ...
   else if (param->getWriteInitialWilsonLines() == 2)
   {
       ...
       for(int ix=0; ix<N; ix++)
        ...
        Outfile1.close();
        Outfile2.close();
        cout<<"wrote " << strVOne_name.str() <<" and " << strVTwo_name.str() << endl;
    } // end binary output
    else
    {
        std::cerr << "# Unknwon option param->getWriteInitialWilsonLines()==" << param->getWriteInitialWilsonLines() << std::endl;
        exit(1); 
    } 
  } 
  // --------
