...
Matrix::Matrix(int n, double a) {
  ...
  for (int i = 0; i < ndim; i++)
    e[i * ndim + i] = complex<double>(a, 0.0);
}
...
void Matrix::reu2() {
  ...
  for (int i = 0; i < 10; i++) {
    E = *this;
    complex<double> N1 = sqrt((conj(e[0]) + e[4]) * conj(conj(e[0]) + e[4]) +
                              (conj(e[3]) - e[1]) * conj(conj(e[3]) - e[1]));
    complex<double> N2 = sqrt((conj(e[0]) + e[8]) * conj(conj(e[0]) + e[8]) +
                              (conj(e[6]) - e[2]) * conj(conj(e[6]) - e[2]));
    complex<double> N3 = sqrt((conj(e[4]) + e[8]) * conj(conj(e[4]) + e[8]) +
...
vector<complex<double>> Matrix::expmCoeff(double *Q, int Nc) {
  ...
  c0 -= (sqrt3 / 2.) * (Q[3] * Q[3] * Q[7] + Q[4] * Q[4] * Q[7] +
                        Q[5] * Q[5] * Q[7] + Q[6] * Q[6] * Q[7]);
  c0 += 3. * (Q[0] * Q[3] * Q[5] + Q[0] * Q[4] * Q[6] + Q[1] * Q[4] * Q[5] -
              Q[1] * Q[3] * Q[6]);
  c0 += 1.5 * (Q[2] * Q[3] * Q[3] + Q[2] * Q[4] * Q[4] - Q[2] * Q[5] * Q[5] -
               Q[2] * Q[6] * Q[6]);
  ...
  for (int i = 0; i < 9; i++) {
    if (std::isnan(result[i].real()) or std::isnan(result[i].imag())) {
      // Sometimes in the very low density region we may encounter (numerically)
      // 0/0 situations In that case, set coefficient to 0, so this contributes
      // only a unit matrix (=vacuum contribution)
      result[i] = 0;
    }
  }
  return result;
...
Matrix &Matrix::expm(double t, const int p) {
  ...
  // Calculate the infinty norm of e, which is defined as the largest row sum of
  // a matrix
  for (int i = 0; i < n; ++i) {
    double temp = 0.0;
    for (int j = 0; j < n; j++)
  ...
  if (n == 2) {
    ...
    H2 *= 1. / (Q(0, 0) * Q(1, 1) * Q(2, 2) + Q(0, 1) * Q(1, 2) * Q(2, 0) +
                Q(0, 2) * Q(1, 0) * Q(2, 1) - Q(0, 2) * Q(1, 1) * Q(2, 0) -
                Q(0, 1) * Q(1, 0) * Q(2, 2) - Q(1, 2) * Q(2, 1) * Q(0, 0));
    // divide by det(H)
  } else {
    cerr << "My matrix exponential works only for up to 3x3 matrices. If you "
...
string Matrix::MatrixToString() {
  ...
  if (n == 3) {
    output << Q(0, 0).real() << " " << Q(0, 0).imag() << " " << Q(1, 0).real()
           << " " << Q(1, 0).imag() << " " << Q(2, 0).real() << " "
           << Q(2, 0).imag() << " " << Q(0, 1).real() << " " << Q(0, 1).imag()
           << " " << Q(1, 1).real() << " " << Q(1, 1).imag() << " "
           << Q(2, 1).real() << " " << Q(2, 1).imag() << " " << Q(0, 2).real()
           << " " << Q(0, 2).imag() << " " << Q(1, 2).real() << " "
           << Q(1, 2).imag() << " " << Q(2, 2).real() << " " << Q(2, 2).imag();
  } else if (n == 2) {
  }
